...
const botAi = require("./server-bot-ai.cjs");
...
function updateHookInteractions(game, dt) {
  ...
  const botIsCommittingToRiftKick = !!(killer?.isBot && killer.bot?.voidRiftAi?.holdingKick);
  if (killer && killer.input.repair && !botIsCommittingToRiftKick && !killer.vault && !killer.breakTarget && !killer.attackState && killer.actionLock <= 0) {
...
const BOT_AI_HELPERS = Object.freeze({
  ...
  riftKickDistance: INTERACT_DISTANCE + 10,
